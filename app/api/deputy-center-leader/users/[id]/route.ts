...
// PUT update user (Deputy Center Leaders cannot edit SUPER_ADMIN, Center_Leader, or Deputy_Center_Leader users)
export async function PUT(
  ...
  try {
    ...
    // Deputy Center Leaders cannot edit SUPER_ADMIN users
    ...
    // Deputy Center Leaders cannot edit Center_Leader users
    ...
    // Deputy Center Leaders cannot edit Deputy_Center_Leader users (prevent editing other deputies or self via this endpoint)
    if (existingUser.role === 'Deputy_Center_Leader') {
      return NextResponse.json(
        { success: false, message: 'You do not have permission to edit Deputy Center Leader users' },
        { status: 403 }
      );
    }

    // Block assigning restricted roles
    ...
    if (primaryRole === 'Deputy_Center_Leader') {
      return NextResponse.json(
        { success: false, message: 'You do not have permission to assign Deputy Center Leader role' },
        { status: 403 }
      );
    }
...
// DELETE user (Deputy Center Leaders cannot delete SUPER_ADMIN, Center_Leader, or Deputy_Center_Leader users)
export async function DELETE(
  ...
  try {
    ...
    // Deputy Center Leaders cannot delete SUPER_ADMIN users
    if (existingUser.role === 'SUPER_ADMIN') {
      return NextResponse.json(
    ...
    // Deputy Center Leaders cannot delete Center_Leader users
    if (existingUser.role === 'Center_Leader') {
      return NextResponse.json(
    ...
    // Deputy Center Leaders cannot delete Deputy_Center_Leader users
    if (existingUser.role === 'Deputy_Center_Leader') {
      return NextResponse.json(
        { success: false, message: 'You do not have permission to delete Deputy Center Leader users' },
        { status: 403 }
      );
    }
