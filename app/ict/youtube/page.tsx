...
export default function YouTubeManagement() {
  ...
  const parseApiResponse = async (response: Response) => {
    ...
    throw new Error(
      `Unexpected response format (${response.status}): ${text.slice(0, 120)}`
    );
  };
  ...
  const fetchVideos = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        showMessage('error', data.message || 'Failed to fetch videos');
      }
    } catch (error) {
      showMessage('error', error instanceof Error ? error.message : 'Error fetching videos');
